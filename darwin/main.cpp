...
static std::string get_runtime_directory() {
...
static int run_gateway_server(const StartupOptions& options) {
    ...
    log_info("[gateway] web UI:  http://" + addr + ":" + std::to_string(port) + "/", show_console);
    log_info("[gateway] models:  http://" + addr + ":" + std::to_string(port) + "/v1/models", show_console);

    if (show_console) {
        std::cout << std::endl;
        std::cout << "To use with Claude Code, set these environment variables before launch:" << std::endl;
        std::cout << "  export ANTHROPIC_BASE_URL=\"http://" << addr << ":" << port << "\"" << std::endl;
        std::cout << "  export ANTHROPIC_AUTH_TOKEN=\"local-gateway\"" << std::endl;
...
static size_t get_current_process_thread_count() {
    // On macOS, use sysctl to count threads for the current process
    pid_t pid = getpid();
    ...
    // Approximate: return 1 as a minimal answer; accurate thread counting
    // on macOS requires iterating /proc or mach APIs which is complex.
